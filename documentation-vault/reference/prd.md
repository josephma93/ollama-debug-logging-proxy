...
OLLAMA_PROXY_LOG_DIR=$HOME/Library/Logs/ollama-proxy
OLLAMA_PROXY_RETENTION_DAYS=10
...
```text
$HOME/bin/ollama-logging-proxy serve
```
...
A LaunchAgent named `dev.ollama.server` must start Ollama using this executable and argument:
...
```text
$HOME/Library/Logs/ollama/stdout.log
$HOME/Library/Logs/ollama/stderr.log
```
...
```xml
<key>Label</key>
<string>dev.ollama.server</string>
...
A LaunchAgent named `dev.ollama.logging-proxy` must start the proxy using the `serve` subcommand:
...
```text
$HOME/Library/Logs/ollama-proxy/stdout.log
$HOME/Library/Logs/ollama-proxy/stderr.log
```

The proxy must write body logs to daily JSONL files in the configured log directory:

```text
$HOME/Library/Logs/ollama-proxy/body-YYYY-MM-DD.jsonl
```
...
```xml
<key>Label</key>
<string>dev.ollama.logging-proxy</string>

<key>ProgramArguments</key>
<array>
  <string>$HOME/bin/ollama-logging-proxy</string>
  <string>serve</string>
</array>
...
<dict>
  ...
  <string>$HOME/Library/Logs/ollama-proxy</string>
...
<string>$HOME/Library/Logs/ollama-proxy/stdout.log</string>

<key>StandardErrorPath</key>
<string>$HOME/Library/Logs/ollama-proxy/stderr.log</string>
```
